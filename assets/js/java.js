...
app.controller('Evento', function ($scope, $http, $rootScope, $routeParams, $filter) {
    ...
    $scope.addf = function () {
        var favo2 = lib.query("favoritos", {
            id: $routeParams.id
        });
        console.log(favo2[0]);
        if (favo2[0] != undefined) {
            lib.deleteRows("favoritos", {
                id: $routeParams.id
            });
            lib.commit();
            $scope.like = false;
        } else {
            lib.insert("favoritos", {
                id: $routeParams.id,
                data: $scope.Evento
            });
            lib.commit();
...
app.controller('Menu', function ($scope) {
    var d = ["Cinema", "Dança", "Teatro Adulto", "Gastronomia", "Crianças", "Literatura", "Artes Visuais", "Exposições", "Artes Manuais", "Meio Ambiente", "Circo", "Cultura Popular", "Cinema e Vídeo", "Artemídia e Cultura Digital", "Música", "Teatro", "Sarau", "Contação de História", "Intervenção", "Alimentação", "Esporte e Atividade Física", "Encontro", "Oficinas", "Musical", "Teatro Infantil", "Teatro de Rua", "Stand Up", "Cultura Oriental", "Oficina", "Instalação", "Geek", "Infantil", "Cabaré"];
    $scope.data = d.sort();
    console.log(1);
});
